...
export default function CatSprite(props) {
  ...
  const Move10 = keyframes`
  from {margin-left: 0px;}
  to   {margin-left: 50px;}
`;

  const TurnLeft15 = keyframes`
...
`;
  ...
  // animation: ${TurnLeft15} 2s linear;
      ...
      activeBlock.map((val) => {
        ...
        switch (val) {
          ...
          case "TurnRight15": {
            RotateRun = styled.div`
              display: inline-block;
              animation: ${TurnRight15} 2s linear;
              position: relative;
            `;
            break;
          }
  ...
  console.log(activeBlock, "pop activeBLock");
